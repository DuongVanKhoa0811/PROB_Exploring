...
python -u main_open_world.py \
    ...
    ${PY_ARGS} --batch_size ${BATCH_SIZE} --pretrain exps/MOWODB/PROB_V13/t1/checkpoint.pth
    
sleep 10    

EXP_DIR=exps/MOWODB/PROB_V14_1_seed_52
PY_ARGS=${@:1}
WANDB_NAME=PROB_MOWODB_V14_1_seed_52
BATCH_SIZE=5
...
python -u main_open_world.py \
    ...
    --wandb_name "${WANDB_NAME}_t1" --exemplar_replay_selection --exemplar_replay_max_length 850\
    --exemplar_replay_dir ${WANDB_NAME} --exemplar_replay_cur_file "learned_owod_t1_ft.txt"\
    ${PY_ARGS} --batch_size ${BATCH_SIZE} --pretrain exps/MOWODB/PROB_V13/t1/checkpoint.pth --seed 52

sleep 10    

EXP_DIR=exps/MOWODB/PROB_V14_1_seed_62
PY_ARGS=${@:1}
WANDB_NAME=PROB_MOWODB_V14_1_seed_62
BATCH_SIZE=5
...
python -u main_open_world.py \
    ...
    --wandb_name "${WANDB_NAME}_t1" --exemplar_replay_selection --exemplar_replay_max_length 850\
    --exemplar_replay_dir ${WANDB_NAME} --exemplar_replay_cur_file "learned_owod_t1_ft.txt"\
    ${PY_ARGS} --batch_size ${BATCH_SIZE} --pretrain exps/MOWODB/PROB_V13/t1/checkpoint.pth --seed 62