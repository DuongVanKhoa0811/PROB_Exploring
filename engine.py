...
from models.prob_extract_obj_features import extract_obj, featureTracker, save_obj_features, save_obj_scores
import h5py
import copy
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, args):
    ...
    obj_temp_str = str(args.obj_temp).replace('.', '_dot_')
    id_file = h5py.File(f'./data/OWOD/ObjFeatures/objscores_V18_Bg_Obj_IoU05_obj_temp_{obj_temp_str}.h5', 'w')
    class_name_file = h5py.File(f'./data/OWOD/ObjFeatures/objscores_V18_Bg_Obj_IoU05_obj_temp_{obj_temp_str}_class_name.h5', 'w')
